...
use crate::{
    cli::extractor::{parse_ip_addr_input, parse_port_input},
    models::connection::Connection,
    server::helper::validate::Validatable,
    thread_executor::thread_fetch_connection_details,
};
...
pub async fn scan<'r>(
    params: ScanParams<'r>,
) -> Result<Json<Vec<Connection>>, (rocket::http::Status, String)> {
    params.validate()?;
    ...
    let ips =
        parse_ip_addr_input(params.target).map_err(|e| (Status::BadRequest, e.to_string()))?;
    let ports = parse_port_input(&params.ports).map_err(|e| (Status::BadRequest, e.to_string()))?;

    let connections =
        thread_fetch_connection_details(&ips, &ports, timeout_ms, concurrency, include_cve)
            .await
