...
pub async fn scan_banner(
...
) -> Result<BannerScanResult, anyhow::Error> {
    ...
    if let Some((header, _body)) = split_request_content(&request_content) {
        Ok(BannerScanResult {
            ...
            web_service: if header.starts_with("HTTP/") {
                WebService::HTTP
            } else {
                WebService::NONE
            },
        ...
        Err(anyhow!("No header in response"))
    }
}

pub async fn read_initial_banner(
    stream: &mut TcpStream,
) -> Result<Option<BannerScanResult>, anyhow::Error> {
...
async fn read_request_buffer_from_tcp_stream(
    stream: &mut TcpStream,
    ...
    loop {
        match stream.read(&mut buffer).await {
            ...
            Err(e) => return Err(anyhow!("Failed to read from stream: {}", e)),
        };
    }
