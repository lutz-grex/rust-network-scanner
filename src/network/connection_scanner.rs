...
use crate::models::connection::{Connection, RequestStatus};
...
pub async fn fetch_connection_details(target: &IpAddr, port: u16, include_banner: bool, timeout: &Duration) -> Result<Connection, Box<dyn Error + Send + Sync>> {
    ...
    let port_scan_data = port_scan_result?;
    let banner_scan_data = banner_scan_result?;
